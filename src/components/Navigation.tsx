            ...
            <div className="relative">
              <button 
                ...
                className={`flex items-center space-x-1 transition-colors ${
                  ['wedding', 'birthday', 'engagement', 'anniversary', 'inauguration', 'corporate', 'farewell', 'proposal', 'festive'].includes(currentPage)
                    ? 'text-wedding-burgundy font-semibold' 
                    : 'text-wedding-brown hover:text-wedding-burgundy'
                }`}
              >
                  ...
                  {services.map((service) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 hover:bg-cream transition-colors flex items-center justify-between ${
                        currentPage === service.page ? 'text-wedding-burgundy font-semibold bg-cream' : 'text-wedding-brown'
                      }`}
                    >
          ...
          <div className="md:hidden py-4 border-t border-wedding-brown/10">
            <div className="flex flex-col space-y-4">
              ...
              <div className="space-y-2">
                <div className="text-wedding-brown font-semibold">Services:</div>
                {services.map((service) => (
                  <button
                    key={service.page}
                    onClick={() => {
                    ...
                    className={`w-full text-left pl-4 py-2 transition-colors flex items-center justify-between ${
                      currentPage === service.page ? 'text-wedding-burgundy font-semibold' : 'text-wedding-brown hover:text-wedding-burgundy'
                    }`}
                  >
                    <span>{service.name}</span>
                    {service.priority && (
